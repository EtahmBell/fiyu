...
export const togetherSessionSchema = z.object({
  ...
  expires_at_for_current_user: nullableString.optional(),
  reveal_ready_expires_at: nullableString.optional(),
  active_for_current_user: z.boolean().optional(),
  partner_key: nullableString.optional(),
...
export const togetherStateSchema = z.object({
  ...
  current_sessions: z.array(togetherSessionSchema).default([]),
...
export const userFiyuSummarySchema = z.object({
  ...
  taste_insights: z.array(z.object({
    ...
    facet_key: z.string(),
    confidence: z.enum(["early", "emerging", "strong", "reliable"]),
    ...
    save_affinity: z.number().min(0).max(1),
    visit_affinity: z.number().min(0).max(1),
  ...
  taste_type: z.object({
    name: z.string(),
    description: z.string(),
    unlocked_at_rating_count: z.number().int().min(20),
    version: z.string(),
  }).nullable(),
