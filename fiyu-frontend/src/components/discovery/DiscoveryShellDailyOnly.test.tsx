...
describe("daily-only discovery shell", () => {
  it.each([
    ...
    [430, 932],
    [768, 1024],
  ])("keeps a bounded map above an independent feed at %ix%i", (width, height) => {
    ...
    expect(screen.getByTestId("mobile-map-region").className).toContain("row-start-1");
    expect(screen.getByTestId("restaurant-scroll-region").className).toContain("row-start-2");
    expect(screen.getByTestId("restaurant-scroll-region").className).toContain("overflow-y-auto");
    expect(screen.getByTestId("mobile-map-region").className).not.toMatch(/fixed|inset-0/);
